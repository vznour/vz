...
/*function getTrendForNode(node){
	...
	var tlen=trend.length;
	 for( var i=0; i < tlen;i++){
        ...
        for (var j=0; j< clen; j++){
        	var v=modelTrend[c[j].i]['s'][i][1];
			var s=modelTrend[c[j].i]['s'][i][2];
        	var r = modelTrend[c[j].i]['s'][i][3];
   ...
   var rc=[];
	 rc['volumeTrend']=[volume];
	 rc['ratingTrend']=[rating];
	 rc['sentimentTrend']=[sentiment];
...
function getModelTrendD3(node){
  ...
  //  sentiment[c[k].n]={type: "line",values:[],key:c[k].n,yAxis: 1};

    if (k==1 || clen < 2){
      volume[node.n]={type: "bar",values:[],key:'All',yAxis: 1};
      rating[node.n]={type: "bar",values:[],key:node.n,yAxis: 1};
     // sentiment[node.n]={type: "area",values:[],key:node.n,yAxis: 1};
    }
	}
	 ...
	 for( var i=0; i < tlen;i++){
      ...
      //  sentiment[node.n].values.push([d,trend[i][2]]);
      for (var j=0; j< clen; j++){
        var v=modelTrend[c[j].i][i][1];
        var r =modelTrend[c[j].i][i][2];
       // console.log("v="+v+",r="+r);
    //   var s= modelTrend[c[j].i]['s'][i][3];


      	volume[c[j].n].values.push([d,v]);
      	rating[c[j].n].values.push([d,r]);
    //  sentiment[c[j].n].values.push([d,s]); 
      // console.log( volume);	
      }
	 }
   var vrc=[];
...
/*
function getTrendD3(node){
    var c =node.c;
    var volume =new Array();
    var sentiment=new Array();
    var rating=new Array();
	  var trend=allTrend;
	if (node.t == MENU_TYPE.CATEGORY ||
      node.t == MENU_TYPE.SUBCATEGORY)
		  trend = modelTrend[node.i];
	var tlen=trend.length;
	for( var i=0; i < tlen;i++){
	 	var d = parseDate(trend[i][0]);
		volume.push([d,trend[i][1]]);
		sentiment.push([d,trend[i][3]]);
		rating.push([d,trend[i][2]]);
	}
	var gdata=[
	   
	{
	  values: rating,     
      key: 'Overall Rating Trend', 
	  type: "line", yAxis: 2
	},
